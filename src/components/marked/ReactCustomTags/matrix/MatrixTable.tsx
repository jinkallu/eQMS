    ...
    <table style={{ border: '1' }}>
      <tbody>
        ...
        {coldata?.label.map((colValue, colIndex) => (
          <tr key={colIndex}>
            <td>{colValue.textContent.trim()}</td> {/* Column data in the first cell of each row */}
            {value && computedStyles && value.label[colIndex].length > 0 && rowdata?.label.map((rowValue, rowIndex) => (
              <td key={`${colIndex}-${rowIndex}`} style={computedStyles[rowIndex][colIndex] ? parseStyles(computedStyles[rowIndex][colIndex] ) : null}>{value?.label?.[rowIndex][colIndex].textContent.trim()}</td>
            ))}
            {value && value.label[colIndex].length === 0 && rowdata?.label.map((rowValue, rowIndex) => (
              <td key={`${colIndex}-${rowIndex}`} >{value?.value?.[rowIndex][colIndex].textContent.trim()}</td>
            ))}
          </tr>
        ))}
      </tbody>
