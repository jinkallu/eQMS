...
//   }
  ...
  function convertR(node, styles, numberingDOM) {
    ...
    const instrText = node.getElementsByTagNameNS(
      wNamespaceURI,
      "instrText"
    )[0]; // Use correct namespace and local name
    if (instrText) {
                  ...
                  for (let k = 0; k < lvls.length; k++) {
                    ...
                    if (ilvl === ilvlVal) {
                      ...
                      const result = docxNumbering.find(
                        (item) =>
                          item.abstractNumIdId === parseInt(abstractNumIdId) &&
                          item.ilvlVal === parseInt(ilvl)
                      );
                      if (result) {
                        //if (parseInt(ilvl) === 0) {
                        for (let l = parseInt(ilvl) + 1; l < 10; l++) {
                          // magic number, must aligh with heading levels
  ...
  function convertTableToHTML(tableElement, styles, numberingDOM) {
    ...
    const tableProperties = tableElement.getElementsByTagNameNS(
      wNamespaceURI,
      "tblPr"
    )[0];
    ...
    for (const rowElement of tableElement.getElementsByTagNameNS(
      wNamespaceURI,
      "tr"
    )) {
      htmlContent += "<tr>";
      // for (const cellElement of rowElement.getElementsByTagNameNS(wNamespaceURI, 'tc')) {
      const cellElements = rowElement.getElementsByTagNameNS(
        wNamespaceURI,
        "tc"
      );
      for (let j = 0; j < cellElements.length; j++) {
        const cellProperties = cellElements[j].getElementsByTagNameNS(
          wNamespaceURI,
          "tcPr"
        )[0];
        const cellStyles =
  ...
  function exctractTableInsideVHStyles(tableProperties) {
    ...
    const wNamespaceURI =
      "http://schemas.openxmlformats.org/wordprocessingml/2006/main";
    const borderStyles = extractInsideVHStyles(
      tableProperties.getElementsByTagNameNS(wNamespaceURI, "tblBorders")[0]
    );
  ...
  function extractInsideVHStyles(bordersElement) {
    ...
    for (const borderType of borderTypes) {
      const borderElement = bordersElement.getElementsByTagNameNS(
        wNamespaceURI,
        borderType
      )[0];
  ...
  function extractTableStyles(tableProperties) {
    ...
    const wNamespaceURI =
      "http://schemas.openxmlformats.org/wordprocessingml/2006/main";
    const borderStyles = extractBorderStyles(
      tableProperties.getElementsByTagNameNS(wNamespaceURI, "tblBorders")[0]
    );
  ...
  function extractBorderStyles(bordersElement) {
    ...
    for (const borderType of borderTypes) {
      ...
      if (borderElement) {
        ...
        if (val && sz && color) {
          borderStyles.push(
            `border-${borderType}: ${val} ${parseInt(sz) / 2}px #${color};`
          );
        }
      }
    }
  ...
  function convertTableCellPropertiesToHTMLStyle(tcPrElement) {
    ...
    const shdElement = tcPrElement.getElementsByTagNameNS(
      wNamespaceURI,
      "shd"
    )[0];
    if (shdElement) {
  ...
  async function handleDocXChange(event) {
