...
const useLinkRecordEvaluator = (state) => {
    ...
    const traverse = async (node) => {
        ...
        switch (node.type) {
            ...
            case "ArrayExpression":
                return await visitArrayExpression(node);


        }
    };

    const visitArrayExpression = async (node) => {
        const array = [];
        for (const element of node.elements) {
            const el = await traverse(element);
            array.push(el);
        }
        return array;
    }
    ...
    return { dependStateIds, types, evaluate };
};

export default useLinkRecordEvaluator;
