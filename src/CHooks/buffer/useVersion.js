...
const useVersion = () => {
  ...
  const getCurrentEditBranch = async (repositoryId, branchName, projectId) => {
    try {
      ...
      const branch = await gitClient.getBranch(
        repositoryId,
        branchName,
        projectId
      );
  ...
  async function getVersionData(projectId, repositoryId, branchName) {
    try {
      ...
      const commits = await getVersionHistory(
        projectId,
        repositoryId,
        branchName
      );
      ...
      setVersionData({
        current: {
          version:
            pullRequests.length === commits.length
              ? commits.length
              : commits.length + "_manipulated",
        },
        history: commits,
      });
    } catch {
  ...
  async function getVersionHistory(projectId, repositoryId, branchName) {
    ...
    let commits = await gitClient.getCommits(
      repositoryId,
      searchCriteria,
      projectId
    );
