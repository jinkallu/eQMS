...
const useCreateBranch = () => {
  ...
  const createBranch = async (projectId, repositoryId, baseBranch, newBranch) => {
    setLoading(true);
    ...
    try {
      ...
      const baseBranchRef = await gitClient.getRefs(repositoryId, projectId, `heads/${baseBranch}`);
      ...
      const result = await gitClient.updateRefs([newBranchRef], repositoryId, projectId);
      setLoading(false);
